...
ggsave("2013graphuse.png", path ="gun_violence")
```
...
```{r Printing Graph}
A
ggsave("2013graph2use.png", path ="gun_violence")
```
...
n_incidentsAAA
library(webshot)
ggsave(plot = n_incidentsAAA, "test.pdf", device = "pdf") 
```
...
permit2 <- final_gun_violence_data %>%
            ...
            mutate(region = ifelse(state %in% c("Wisconsin", "Michigan", "Ohio", "Indiana", "Illinois", "Iowa", "Kansas", "Minnesota", "Missouri", "Nebraska", "North Dakota", "South Dakota", "midwest",
                            ...
                            ifelse(state %in% c("Arizona", "Colorado", "Idaho", "New Mexico", "Montana", "Utah", "Nevada", "Wyoming", "Alaska", "California", "Hawaii", "Oregon", "Washington", "west", "something")))))))))
```
...
library(usmap)
plot_usmap(data = final_gun_violence_data, values = nrow(final_gun_violence_data), regions = "counties", size = 0.05) +
...
ggsave("2014graphuse.png", path ="gun_violence")
```
...
ggsave("2014graph2use.png", path ="gun_violence")
```
...
ggsave("2015graphuse.png", path ="gun_violence")
```
...
ggsave("2015graph2use.png", path ="gun_violence")
```
...
final_gun_violence_data %>%
  ...
  labs(x = "Month", 
       ...
       subtitle = "Do Gun Permits Sold Increase in Summer Months?") +
  scale_y_continuous(labels = comma)
ggsave("2016graphuse.png", path ="gun_violence")
```
...
```{r Printing Graph4}
D
ggsave("2016graph2use.png", path ="gun_violence")
```
...
final_gun_violence_data %>%
  ...
  labs(x = "Month", 
       ...
       subtitle = "Do Gun Permits Sold Increase in Summer Months?") +
  scale_y_continuous(labels = comma)
ggsave("2017graphuse.png", path ="gun_violence")
```
...
```{r Printing Graph5}
E
ggsave("2017graph2use.png", path ="gun_violence")
```
...
# dividing_incidents_population <- permit1 %>%
#   mutate(division = (n_incidents/population) * 10000000)
# 
# m2 <- lm(division ~ permit, data = dividing_incidents_population)

object <- data.frame(permit = seq(min(permit1$permit), max(permit1$permit), length.out = 100), population = mean(permit1$population))

object$prediction <- predict.lm(m1, object, interval = c("confidence"))
# worked on above with Georgie - object 'cons' not found --> get it to work then the aesthetic error shows up

summary(m2)

# predicts n_incidents as a function of permit controlling for population

object %>%
ggplot(aes(x=permit, y=prediction)) +
  geom_point()
```

```{r}
library(coefplot)
coefplot(m1, drop(cons))
```
